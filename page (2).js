      ...
      <div style={{ maxWidth: 480, margin: "0 auto" }}>
        ...
        <section style={cardStyle}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <input placeholder="Phone number" value={phone} onChange={(e) => setPhone(e.target.value)} style={{ ...inputStyle, marginTop: 0 }} />
            <button onClick={searchCustomer} style={{ ...buttonStyle, width: "auto", marginTop: 0, padding: "10px 16px" }}>Find</button>
          </div>

          {customerData?.customer && (
            <div style={{ marginTop: 12 }}>
              <p style={{ margin: 0 }}><strong>{customerData.customer.phone}</strong></p>
                ...
                <div style={{ marginTop: 8 }}>
                  ...
                  {customerData.recentOrders.filter((o) => o.status === "PENDING").map((o) => (
                    <div key={o.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      <span style={{ fontSize: 13 }}>{o.order_number} — ₹{o.total_amount}</span>
        ...
        <section style={cardStyle}>
          ...
          <button onClick={createOrder} style={buttonStyle} disabled={!phone || Object.keys(cart).length === 0}>
