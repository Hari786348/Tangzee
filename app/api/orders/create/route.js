...
// POST { phone: "9876543210", items: [{ dessertId, quantity }] }
...
export async function POST(req) {
  ...
  if (!phone || !items || items.length === 0) {
    return NextResponse.json({ error: "PHONE_AND_ITEMS_REQUIRED" }, { status: 400 });
  }
  ...
  if (!customer) {
    const { data: created, error: createErr } = await db.from("customers").insert({ phone }).select().single();
